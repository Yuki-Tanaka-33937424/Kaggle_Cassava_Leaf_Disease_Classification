...
ファイル
train/test]_画像ファイルをイメージします。テスト画像のフルセットは、スコアリングのために提出されたときにのみノートブックに表示されます。テストセットには約15,000枚の画像が含まれています。
...
予測された疾患のIDコードをラベル付けします。

train/test]_tfrecords 画像ファイルをtfrecord形式で保存します。
